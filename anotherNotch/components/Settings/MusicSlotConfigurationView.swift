                            ...
                            VStack(spacing: 6) {
                                ...
                                .onTapGesture {
                                    if let idx = musicControlSlots.firstIndex(of: .none) {
                                        ...
                                        withAnimation { updateSlot(control, at: min(musicControlSlots.count, fixedSlotCount - 1)) }
                                    }
                                }
        ...
        if raw.hasPrefix("slot:") {
            ...
            if from < slots.count {
                if toIndex >= slots.count {
                    slots.append(contentsOf: Array(repeating: .none, count: toIndex - slots.count + 1))
                }
