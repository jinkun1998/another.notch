    ...
    var body: some Scene {
        MenuBarExtra(isInserted: $showMenuBarIcon) {
            ...
            Image("MenuBarIcon")
                ...
                .accessibilityLabel("anotherNotch")
        }
    }
}
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var window: NSWindow?
    let vm: AnotherNotchViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? AnotherNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? AnotherNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createAnotherNotchWindow(
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = AnotherNotchViewModel(screenUUID: uuid)
                    let window = createAnotherNotchWindow(for: screen, with: viewModel)
